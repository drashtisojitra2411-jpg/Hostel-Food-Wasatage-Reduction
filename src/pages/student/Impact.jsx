...
export default function Impact() {
    ...
    const [targets, setTargets] = useState({ co2: 0, money: 0, people: 0, meals: 0, trees: 0 });
    const growthData = [
        { month: 'MEALS', value: Math.min(100, Math.round(targets.meals)) || 0, icon: '🌱' },
        { month: 'PEOPLE', value: Math.min(100, Math.round(targets.people)) || 0, icon: '🌿' },
        { month: 'MONEY', value: Math.min(100, Math.round(targets.money / 100)) || 0, icon: '🌳' },
        { month: 'CO2', value: Math.min(100, Math.round(targets.co2 * 2)) || 0, icon: '🌲', active: true }
    ];
    ...
    useEffect(() => {
        async function loadImpact() {
            try {
                ...
                const co2 = Number(res?.wastes_prevented_kg || 0);
                const meals = Number(res?.meals_booked || 0);
                const people = Number(res?.donations_completed || 0);
                const money = Math.round(co2 * 340);
                const treeCount = Math.max(0, Math.round(co2 / 2.5));
                setTargets({ co2, money, people, meals, trees: treeCount });
            } catch {
                setTargets({ co2: 0, money: 0, people: 0, meals: 0, trees: 0 });
            }
        }
        loadImpact();
    }, []);
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            setAnimatedValues({
                co2: targets.co2 * easeOut,
                money: targets.money * easeOut,
                people: targets.people * easeOut,
                        ...
                        <Card variant="premium" className="border-white/10 p-16 bg-black group-hover:ring-1 ring-creative-lime/30 transition-all duration-700">
                            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-12 relative z-10">
                                ...
                                <div className="text-left xl:text-right">
                                    <div className="text-7xl font-black italic text-creative-lime leading-tight">{Math.min(100, Math.round(animatedValues.meals > 0 ? (animatedValues.meals / (targets.meals || 1)) * 100 : 0))}%</div>
                            ...
                            <div className="mt-16 h-4 bg-white/5 rounded-full overflow-hidden p-1 ring-1 ring-white/10">
                                <div className="h-full bg-gradient-to-r from-creative-lime via-creative-lime to-white relative animate-shimmer rounded-full" style={{ width: `${Math.min(100, Math.round(animatedValues.meals > 0 ? (animatedValues.meals / (targets.meals || 1)) * 100 : 0))}%` }}>
                            ...
                            <div className="flex justify-between mt-8 text-[10px] font-black tracking-widest text-white/20 uppercase font-mono">
                                ...
                                <span>TERMINAL TARGET: {(targets.co2 || 0).toFixed(2)} KG</span>
                            </div>
                        </Card>
                    </div>
